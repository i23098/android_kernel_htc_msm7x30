...
static int aic3254_config(CODEC_SPI_CMD *cmds, int size)
{
	...
	if (!codec_dev) {
		printk(KERN_ERR "%s: no spi device\n", __func__);
		return -EFAULT;
	}

	if (cmds == NULL) {
		printk(KERN_ERR "%s: invalid spi parameters\n", __func__);
		return -EINVAL;
	}
	...
	if (suspend_flag && ctl_ops->panel_sleep_in) {
		...
		suspend_flag = 0;
		if (ret < 0)
			printk(KERN_ERR "%s: cannot make panel awake,"
				"it might failed on transmit SPI command\n"
				, __func__);
		else
			printk(KERN_INFO "%s: success on invoking panel_sleep_in\n"
				, __func__);
	}
		...
		for (i = 0; i < size; i++) {
			...
			case 'r':
				for (retry = AIC3254_MAX_RETRY; retry > 0; retry--) {
					...
					if (ret < 0)
						printk(KERN_ERR "%s: read fail %d, retry\n",
							__func__, ret);
				...
				if (retry <= 0)
					printk(KERN_INFO "3254 power down procedure"
...
static int aic3254_config_ex(CODEC_SPI_CMD *cmds, int size)
{
	...
	if (spi_t_cmds == NULL) {
		printk(KERN_ERR "%s: kmalloc spi transfer struct fail\n", __func__);
		goto error;
	} else
	...
	if (buffer == NULL) {
		printk(KERN_ERR "%s: kmalloc buffer fail\n", __func__);
		goto error;
	} else
...
static void aic3254_tx_config(int mode)
{
	...
	if (mode != UPLINK_OFF && mode != POWER_OFF) {
		/* uplink_Wakeup */
		printk(KERN_INFO "uplink wakeup len(%d)\n",
	...
	/* route tx device */
	printk(KERN_INFO "uplink TX %d len(%d)\n", mode,
...
static void aic3254_rx_config(int mode)
{
	...
	if (mode != DOWNLINK_OFF && mode != POWER_OFF) {
		/* Downlink Wakeup */
		printk(KERN_INFO "downlink wakeup len(%d)\n",
	...
	/* route rx device */
	printk(KERN_INFO "downlink RX %d len(%d)\n", mode,
...
static void aic3254_powerdown(void)
{
	...
	if (aic3254_uplink != NULL) {
		printk(KERN_INFO "power off AIC3254 len(%d)++\n",
	...
	if (drv->enabled) {
		...
		drv->enabled = 0;
		printk(KERN_INFO "%s: disable CLK\n", __func__);
	}
#endif
	...
	printk(KERN_INFO "power off AIC3254 %lldms --\n", t2);
	return;
}
...
static void aic3254_loopback(int mode)
{
	if (!(ctl_ops->lb_dsp_init &&
		...
		printk(KERN_INFO "%s: AIC3254 LOOPBACK not supported\n", __func__);
		return;
	}
	...
	printk(KERN_INFO "%s: set AIC3254 in LOOPBACK mode\n", __func__);
	switch (mode) {
	case 0:
		/* receiver v.s. imic */
	...
	case 13:
		...
		if (ctl_ops->lb_receiver_bmic)
			...
			printk(KERN_INFO "%s: receiver v.s. 2nd mic loopback not supported\n", __func__);
		break;
	...
	case 14:
		...
		if (ctl_ops->lb_speaker_bmic)
			...
			printk(KERN_INFO "%s: speaker v.s. 2nd mic loopback not supported\n", __func__);
		break;
	...
	case 15:
		...
		if (ctl_ops->lb_headset_bmic)
			...
			printk(KERN_INFO "%s: headset v.s. 2nd mic loopback not supported\n", __func__);
		break;
	default:
		break;
	}
...
{
	printk(KERN_INFO "%s: (%d,%d) uses 3254 default setting\n", __func__, path, en);
	if (en) {
		/* Downlink_Wakeup */
...
static int aic3254_set_config(int config_tbl, int idx, int en)
{
	...
	case AIC3254_CONFIG_MEDIA:
		...
		printk(KERN_INFO "%s: configure miniDSP index(%d) len = %d ++\n",
			__func__, idx, len);
		printk(KERN_INFO "%s: rx mode %d, tx mode %d\n",
		...
		printk(KERN_INFO "%s: configure miniDSP index(%d) time: %lldms --\n",
			__func__, idx, (t2));
		break;
	}
...
void aic3254_set_mode(int config, int mode)
{
	...
	case AIC3254_CONFIG_TX:
		/* TX */
		printk(KERN_INFO "%s: AIC3254_CONFIG_TX mode = %d\n",
			__func__, mode);
	...
	case AIC3254_CONFIG_RX:
		/* RX */
		printk(KERN_INFO "%s: AIC3254_CONFIG_RX mode = %d\n",
			__func__, mode);
	   ...
	   unsigned long argc)
{
	...
	if (aic3254_uplink == NULL ||
		...
		printk(KERN_ERR "%s: cmd 0x%x, invalid pointers\n", __func__, cmd);
		return -EFAULT;
	}
	...
	case AIC3254_SET_RX_PARAM:
		if (copy_from_user(&para, (void *)argc, sizeof(para))) {
			printk(KERN_ERR "%s: failed on copy_from_user\n", __func__);
			return -EFAULT;
		}

		printk(KERN_INFO "%s: parameters(%d, %d, %p)\n", __func__,
		...
		if (para.row_num > IO_CTL_ROW_MAX
			...
			printk(KERN_ERR "%s: data size mismatch with allocated"
					" memory (%d,%d)\n", __func__,
		...
		printk(KERN_INFO "%s: update table(%d,%d) successfully\n",
	...
	case AIC3254_CONFIG_MEDIA:
		...
		if (ret < 0)
			printk(KERN_ERR "%s: configure(%d) error %d\n",
				__func__, i, ret);
		break;
	case AIC3254_CONFIG_VOLUME_L:
		...
		if (volume < -127 || volume > 48) {
			printk(KERN_ERR "%s: volume out of range\n", __func__);
			return -EFAULT;
		}

		printk(KERN_INFO "%s: AIC3254 config left volume %d\n",
				__func__, volume);
	...
	case AIC3254_CONFIG_VOLUME_R:
		...
		printk(KERN_INFO "%s: AIC3254 config right volume %d\n",
				__func__, volume);
	...
	case AIC3254_DUMP_PAGES:
		...
		if (i > AIC3254_MAX_PAGES) {
			printk(KERN_ERR "%s: invalid page number %d\n", __func__, i);
			return -EINVAL;
		}

		printk(KERN_INFO "========== %s: dump page %d ==========\n",
				__func__, i);
		...
		for (i = 0; i < AIC3254_MAX_REGS; i++) {
			...
			if (ret < 0)
				printk(KERN_ERR "read fail on register 0x%X\n", i);
			else
	...
	case AIC3254_WRITE_REG:
		...
		printk(KERN_INFO "%s: command list (%c,%02X,%02X) (%c,%02X,%02X)\n",
	...
	case AIC3254_READ_REG:
		...
		if (copy_to_user((void *)argc, &reg, sizeof(CODEC_SPI_CMD)*2)) {
			printk(KERN_ERR "%s: failed on copy_to_user\n", __func__);
			return -EFAULT;
		}
		break;
	...
	case AIC3254_LOOPBACK:
		...
		printk(KERN_INFO "%s: index %d for LOOPBACK\n", __func__, i);
		aic3254_loopback(i);
		break;
	default:
...
static int __init spi_aic3254_init(void)
{
	...
	if (IS_ERR(codec_drv->rx_mclk)) {
		printk(KERN_ERR "%s:failed to get mi2s mclk\n", __func__);
	...
	if (IS_ERR(codec_drv->rx_sclk)) {
		printk(KERN_ERR "%s:failed to get mi2s sclk\n", __func__);
		misc_deregister(&aic3254_misc);
		spi_unregister_driver(&spi_aic3254);
