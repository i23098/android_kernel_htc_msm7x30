...
static int sd_blk_rw_rq_prep(struct mmc_queue_req *mqrq,
			       struct mmc_card *card,
			       int disable_multi,
			       struct mmc_queue *mq)
{
...
static int sd_blk_issue_rw_rq(struct mmc_queue *mq, struct request *req)
{
	...
	struct mmc_blk_request *brq = &mq->mqrq_cur->brq;
	int ret = 1, disable_multi = 0, retry = 0;
	int try_recovery = 1, do_reinit = 0, do_remove = 0, err = 0;
	int card_no_ready = 0;

	do {
		if (sd_blk_rw_rq_prep(mq->mqrq_cur, card, disable_multi, mq) == 0)
			return 0;
