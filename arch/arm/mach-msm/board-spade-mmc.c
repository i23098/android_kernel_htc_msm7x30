...
int spade_wifi_power(int on)
{
	...
	gpio_set_value(SPADE_GPIO_WIFI_SHUTDOWN_N, on); /* WIFI_SHUTDOWN */
	mdelay(120);
	return 0;
}
...
int __init spade_init_mmc(unsigned int sys_rev)
{
	...
	*/
	...
	id = PCOM_GPIO_CFG(SPADE_GPIO_WIFI_SHUTDOWN_N, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),
	msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);
	gpio_set_value(SPADE_GPIO_WIFI_SHUTDOWN_N, 0);

	msm_add_sdcc(3, &spade_wifi_data);
	...
	if (opt_disable_sdcard) {
		printk(KERN_INFO "spade: SD-Card interface disabled\n");
		goto done;
	}
