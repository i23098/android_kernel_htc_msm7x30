...
static void lpa_reset(struct lpa_drv *lpa)
{
	...
	if (!adsp_clk) {
		printk(KERN_ERR "failed to get adsp clk\n");
		goto error;
	}
	clk_enable(adsp_clk);
...
struct lpa_drv *lpa_get(void)
{
	...
	if (the_lpa_state.assigned) {
		printk(KERN_ERR "LPA HW accupied\n");
	...
	struct lpa_codec_config *config_ptr)
{
	...
	case 1:
		...
		break;
	default:
		printk(KERN_ERR "unsupported number of channel\n");
		goto error;
	}
	...
	case 16:
		...
		break;
	default:
		printk(KERN_ERR "unsupported sample width \n");
		goto error;
	}
...
int lpa_cmd_enable_codec(struct lpa_drv *lpa, bool enable)
{
	...
	if (enable) {
		...
		if (val & LPA_OBUF_CODEC_CODEC_INTF_EN_BMSK) {
			...
			printk(KERN_ERR "LPA codec is already disable\n");
	}
	return 0;
}
...
static int lpa_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "lpa probe\n");

	if (!pdev || !pdev->dev.platform_data) {
		printk(KERN_ERR "no plaform data\n");
		rc = -ENODEV;
		goto error;
	}
	...
	if (!mem_src) {
		printk(KERN_ERR "LPA base address undefined\n");
		rc = -ENODEV;
		goto error;
	}
