...
int aux_pcm_gpios_request(void)
{
	...
	if (rc) {
		printk(KERN_ERR "GPIO request for AUX PCM DOUT failed\n");
		return rc;
	}

	rc = gpio_request(the_aux_pcm_state.din, "AUX PCM DIN");
	if (rc) {
		printk(KERN_ERR "GPIO request for AUX PCM DIN failed\n");
	...
	if (rc) {
		printk(KERN_ERR "GPIO request for AUX PCM SYNC OUT failed\n");
		gpio_free(the_aux_pcm_state.dout);
	...
	if (rc) {
		printk(KERN_ERR "GPIO request for AUX PCM CLKIN A failed\n");
		gpio_free(the_aux_pcm_state.dout);
		gpio_free(the_aux_pcm_state.din);
...
static int get_aux_pcm_gpios(struct platform_device *pdev)
{
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,
					"aux_pcm_dout");
	if  (!res) {
		printk(KERN_ERR "%s: failed to get gpio AUX PCM DOUT\n", __func__);
		return -ENODEV;
	}
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,
					"aux_pcm_din");
	if  (!res) {
		printk(KERN_ERR "%s: failed to get gpio AUX PCM DIN\n", __func__);
		return -ENODEV;
	}
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,
					"aux_pcm_syncout");
	if  (!res) {
		printk(KERN_ERR "%s: failed to get gpio AUX PCM SYNC OUT\n", __func__);
		return -ENODEV;
	}
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,
					"aux_pcm_clkin_a");
	if  (!res) {
		printk(KERN_ERR "%s: failed to get gpio AUX PCM CLKIN A\n", __func__);
		return -ENODEV;
	}
...
static int aux_pcm_probe(struct platform_device *pdev)
{
	...
	if (rc) {
		printk(KERN_ERR "GPIO configuration failed\n");
		rc = -ENODEV;
	}
