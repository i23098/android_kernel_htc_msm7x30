			    ...
			    void (*getevent)(void *ptr, size_t len))
{
	...
	case AFE_APU_MSG_VOC_TIMING_SUCCESS:
		printk(KERN_INFO "Received VOC_TIMING_SUCCESS message from AFETASK\n");
		break;
	case ADSP_MESSAGE_ID:
		MM_DBG("Received ADSP event: module enable/disable(audpptask)");
		break;
	default:
		printk(KERN_INFO "unexpected message from afe \n");
	}
...
int afe_enable(u8 path_id, struct msm_afe_config *config)
{
	...
	if (path_id < AFE_HW_PATH_CODEC_RX ||
		...
		printk(KERN_ERR "invalid path id %d\n", path_id);
		return -EINVAL;
	}
	...
	if (!afe->in_use && !afe->aux_conf_flag) {
		...
		if (rc < 0) {
			printk(KERN_ERR "%s: failed to get AFETASK module\n", __func__);
			goto error_adsp_get;
		}
		printk(KERN_INFO "Enabling msm_adsp_enable");
	...
	if (!rc) {
		printk(KERN_ERR "AFE failed to respond within %d ms\n", AFE_MAX_TIMEOUT);
		rc = -ENODEV;
		if (!afe->in_use) {
...
static int __init afe_init(void)
{
	...
	printk(KERN_INFO "AFE driver init\n");
...
{
	printk(KERN_INFO "AFE driver exit\n");
