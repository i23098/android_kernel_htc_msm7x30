...
int audpp_send_queue3(void *cmd, unsigned len)
{
	...
	while (retry--) {
		...
		if (result) {
			printk(KERN_ERR "ACDB=> Failed to send audpp_send_queue3 to postproc, retry %d\n", 5 - retry);
			mdelay(20);
		} else
			break;
	}
			    ...
			    void (*getevent) (void *ptr, size_t len))
{
	...
	case AUDPP_MSG_CFG_MSG:
		if (msg[0] == AUDPP_MSG_ENA_ENA) {
			...
			printk(KERN_ERR "invalid config msg %d\n", msg[0]);
		}
		break;
	...
	case AUDPP_MSG_AVSYNC_MSG:
		...
		break;
	default:
		printk(KERN_INFO "unhandled msg id %x\n", id);
	}
}
...
int audpp_enable(int id, audpp_event_func func, void *private)
{
	...
	if (audpp->open_count++ == 0) {
		...
		if (res < 0) {
			printk(KERN_ERR "audpp: cannot open AUDPPTASK\n");
			audpp->open_count = 0;
			audpp->func[id] = NULL;
...
void audpp_disable(int id, void *private)
{
	...
	if (--audpp->open_count == 0) {
		...
		rc = wait_event_interruptible(audpp->event_wait,
				(audpp->enabled == 0));
		if (audpp->enabled == 0)
			printk(KERN_INFO "Received CFG_MSG_DISABLE from ADSP\n");
		else
			printk(KERN_ERR "Didn't receive CFG_MSG DISABLE \
					message from ADSP\n");
		audpp->enabled = 0;
			...
			enum obj_type objtype)
{
	...
	if (objtype) {
		if (id > 5) {
			printk(KERN_ERR "Wrong POPP decoder id: %d\n", id);
			return -EINVAL;
		}
	} else {
		if (id > 4) {
			printk(KERN_ERR "Wrong COPP decoder id: %d\n", id);
			return -EINVAL;
		}
	}
		     ...
		     unsigned *queueid)
{
	...
	if (audpp->codec_cnt[codec_type] < audpp->decoder_count) {
		...
		if (audpp->decoder_count > (max_instance + 1)) {
			printk(KERN_ERR "Can not support, already reached max\n");
			audpp->decoder_count--;
			audpp->codec_cnt[codec_type]--;
			goto done;
		}
	...
	if (lidx < audpp->dec_database->num_dec) {
		...
		*queueid =
		...
		printk(KERN_INFO "decid =0x%08x module_name=%s, queueid=%d \n", decid,
				*module_name, *queueid);
	}
done:
				 ...
				 const char *buf, size_t count)
{
	...
	mutex_lock(audpp->lock_dec);
	if (audpp->dec_inuse) {
		printk(KERN_ERR "Can not change profile, while playback in progress\n");
		goto done;
	}
	...
	if (!rc &&
		...
		printk(KERN_ERR "Not a valid Concurrency case\n");
		rc = -EINVAL;
	}
done:
	mutex_unlock(audpp->lock_dec);
...
static int audpp_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "Number of decoder supported  %d\n",
		audpp->dec_database->num_dec);
	printk(KERN_INFO "Number of concurrency supported  %d\n",
