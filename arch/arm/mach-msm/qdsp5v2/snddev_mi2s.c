...
static int snddev_mi2s_open_tx(struct msm_snddev_info *dev_info)
{
	...
	if (snddev_mi2s_data->channel_mode == 2) {
		...
		printk(KERN_ERR "%s: Invalid number of channels = %u\n", __func__,
	...
	if (IS_ERR_VALUE(rc)) {
		printk(KERN_ERR "%s: afe_enable failed for AFE_HW_PATH_MI2S_TX "
		       "rc = %d\n", __func__, rc);
		return -ENODEV;
	}
...
static int snddev_mi2s_open(struct msm_snddev_info *dev_info)
{
	...
	if (!dev_info) {
		printk(KERN_ERR "%s:  msm_snddev_info is null \n", __func__);
		return -EINVAL;
	}
	...
	if (drv->sd_lines_used & snddev_mi2s_data->sd_lines) {
		printk(KERN_ERR "%s: conflict in SD data line. can not use the device\n",
		       __func__);
	...
	if (!drv->clocks_enabled) {
		...
		if (rc) {
			printk(KERN_ERR "%s: mi2s GPIO config failed for %s\n",
			       __func__, snddev_mi2s_data->name);
			mutex_unlock(&drv->lock);
			return -EIO;
		}
	...
	if (snddev_mi2s_data->capability & SNDDEV_CAP_RX) {
		...
		if (IS_ERR_VALUE(rc)) {
			printk(KERN_ERR " snddev_mi2s_open_rx failed \n");
		...
		if (IS_ERR_VALUE(rc)) {
			printk(KERN_ERR " snddev_mi2s_open_tx failed \n");
			goto mi2s_cleanup_open;
		}
...
static int snddev_mi2s_close(struct msm_snddev_info *dev_info)
{
	...
	snddev_mi2s_data = dev_info->private_data;
	if (!dev_info->opened) {
		printk(KERN_ERR " %s: calling close device with out opening the"
		       " device \n", __func__);
		return -EIO;
	}
...
static int snddev_mi2s_probe(struct platform_device *pdev)
{
	...
	if (!dev_info) {
		printk(KERN_ERR "%s: uneable to allocate memeory for msm_snddev_info \n",
		       __func__);
...
static int __init snddev_mi2s_init(void)
{
	...
	if (IS_ERR(drv->mclk)) {
		printk(KERN_ERR "%s:  clk_get mi2s_mclk failed  \n", __func__);
		goto error_mclk;
	}

	drv->sclk = clk_get(NULL, "mi2s_s_clk");
	if (IS_ERR(drv->sclk)) {
		printk(KERN_ERR "%s:  clk_get mi2s_sclk failed  \n", __func__);
