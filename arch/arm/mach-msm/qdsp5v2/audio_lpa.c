...
{
	printk(KERN_INFO "get lpa session\n");
	return lpa_enable;
}
...
static int audio_enable(struct audio *audio)
{
	...
	if (msm_adsp_enable(audio->audplay)) {
		printk(KERN_ERR "msm_adsp_enable(audplay) failed\n");
		return -ENODEV;
	}
			      ...
			      void (*getevent) (void *ptr, size_t len))
{
	...
	case ADSP_MESSAGE_ID:
		...
		break;
	default:
		printk(KERN_ERR "unexpected message from decoder\n");
		break;
	}
}
...
static void audio_dsp_event(void *private, unsigned id, uint16_t *msg)
{
	...
	case AUDPP_MSG_STATUS_MSG:{
			...
			case AUDPP_DEC_STATUS_EOS:
				...
				break;
			default:
				printk(KERN_ERR "unknown decoder status\n");
				break;
			}
			break;
		}
...
static void audlpa_async_send_buffer(struct audio *audio)
{
	...
	//printk(KERN_ERR "audlpa_async_send_buffer -audio->device_switch:%d,audio->drv_status:%d,audio->bytecount_head =%lld"
	...
	if (audio->device_switch == DEVICE_SWITCH_STATE_NONE) {
		...
		if (next_buf) {
			...
			((signed)(next_buf->buf.data_len - temp) >= 0)) {
				...
				cmd.buf_ptr	= (unsigned) next_buf->paddr;
				//printk(KERN_ERR "cmd.buf_ptr=%d",cmd.buf_ptr);
				cmd.buf_size = next_buf->buf.data_len >> 1;
				//printk(KERN_ERR "cmd.buf_size=%d",cmd.buf_size);
				cmd.partition_number	= 0;
				audio->bytecount_given = audio->bytecount_head + next_buf->buf.data_len;
				//printk(KERN_ERR "roger: audio->bytecount_given=%lld",audio->bytecount_given);
				wmb();
				audplay_send_queue0(audio, &cmd, sizeof(cmd));
				audio->out_needed = 0;
				audio->drv_status |= ADRV_STATUS_OBUF_GIVEN;
				...
				uint32_t *payload)
{
	...
	if (needed && !audio->wflush) {
		...
		if (audio->device_switch !=
			...
			//list_for_each_entry(next_buf, &audio->out_queue, list)
			//printk(KERN_ERR "audlpa_async_send_data :- popping out a buffer- next_buf-data_Len:%d,next_buf-addr: %lu",
			//		   next_buf->buf.data_len,(unsigned long)next_buf->paddr);

				audio->bytecount_head += used_buf->buf.data_len;
				//printk(KERN_ERR "audio->bytecount_head updated- %llu",audio->bytecount_head);
				temp = audio->bytecount_head;
...
static long audlpa_process_event_req(struct audio *audio, void __user *arg)
{
	...
	if (drv_evt) {
		...
		printk(KERN_ERR "%s: fail to find event\n", __func__);
		spin_unlock_irqrestore(&audio->event_queue_lock, flags);
		return -1;
	}
		...
		void *vaddr, unsigned long len)
{
	...
	list_for_each_entry(region_elt, &audio->pmem_region_queue, list) {
		if (CONTAINS(region_elt, &t) || CONTAINS(&t, region_elt) ||
			...
			printk(KERN_ERR "region (vaddr %p len %ld)"
		     ...
		     unsigned long len, struct audlpa_pmem_region **region)
{
	...
	*region = NULL;
	...
	if (match_count > 1) {
		printk(KERN_ERR "multiple hits for vaddr %p, len %ld\n", addr, len);
		    ...
		    unsigned long len, int ref_up)
{
	...
	if (ret) {
		printk(KERN_ERR "lookup (%p, %ld) failed\n", addr, len);
		return 0;
	}
	if (ref_up)
		region->ref_cnt++;
	...
	void __user *arg)
{
	...
	if (dir) {
		...
		//list_for_each_entry(next_buf, &audio->out_queue, list)
		//	printk(KERN_ERR "audlpa_aio_buf_add :- Adding a buffer to list - buf_node-data_Len:%d,buf_node-addr: %lu",
		//		   next_buf->buf.data_len,(unsigned long)next_buf->paddr);
		spin_unlock_irqrestore(&audio->dsp_lock, flags);
...
static long audio_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_FLUSH:
		...
		if (audio->running) {
			...
			rc = wait_event_interruptible(audio->write_wait,
				!audio->wflush);
			if (rc < 0) {
				printk(KERN_ERR "AUDIO_FLUSH interrupted\n");
				rc = -EINTR;
			}
		} else {
			audio->wflush = 0;
	...
	case AUDIO_SET_CONFIG:{
		...
		printk(KERN_INFO "AUDIO_SET_CONFIG\n");
		...
		if (config.channel_count == 1) {
			...
			rc = -EINVAL;
			printk(KERN_INFO "ERROR: config.channel_count == %d\n",
		...
		if (config.bits == 8)
			...
			rc = -EINVAL;
			printk(KERN_INFO "ERROR: config.bits == %d\n", config.bits);
			break;
		}
...
int audlpa_async_fsync(struct audio *audio)
{
	...
	//printk(KERN_ERR "audlpa_async_fsync after wait_event_interruptible_timeout rc =%d",rc);
	if (rc < 0)
		goto done;
...
static int audio_release(struct inode *inode, struct file *file)
{
	...
	printk(KERN_INFO "audio instance 0x%08x freeing\n", (int)audio);
	mutex_lock(&audio->lock);
	...
	union msm_audio_event_payload payload)
{
	...
	if (!list_empty(&audio->free_event_queue)) {
		...
		if (!e_node) {
			printk(KERN_ERR "No mem to post event %d\n", type);
			return;
		}
	}
...
static int audio_open(struct inode *inode, struct file *file)
{
	...
	if (!audio) {
		printk(KERN_ERR "no memory to allocate audio instance\n");
		rc = -ENOMEM;
		goto done;
	}
	printk(KERN_INFO "audio instance 0x%08x created\n", (int)audio);
	...
	if (audio->minor_no >= ARRAY_SIZE(audlpa_decs)) {
		printk(KERN_ERR "incorrect minor_no %d\n", audio->minor_no);
		kfree(audio);
		goto done;
	}
	...
	decid = audpp_adec_alloc(dec_attrb, &audio->module_name,
			&audio->queue_id);
	if (decid < 0) {
		printk(KERN_ERR "No free decoder available\n");
		rc = -ENODEV;
		printk(KERN_INFO "audio instance 0x%08x freeing\n", (int)audio);
		kfree(audio);
		goto done;
	...
	if (rc) {
		printk(KERN_ERR "failed to get %s module\n", audio->module_name);
		goto err;
	}
	...
	rc = auddev_register_evt_listner(audio->device_events,
					...
					(void *)audio);
	if (rc) {
		printk(KERN_ERR "%s: failed to register listnet\n", __func__);
		goto event_err;
	}
	...
	for (i = 0; i < AUDLPA_EVENT_NUM; i++) {
		...
		if (e_node)
			...
			printk(KERN_ERR "event pkt alloc failed\n");
			break;
		}
	}
	lpa_enable = 1;
