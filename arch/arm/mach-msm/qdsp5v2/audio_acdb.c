...
static int htc_acdb_init(char *filename)
{
	...
	printk(KERN_INFO "acdb: load '%s'\n", filename);
	if (request_firmware(&fw, filename, acdb_misc.this_device) < 0) {
		printk(KERN_INFO "acdb: load 'default.acdb' failed...\n");
		return -ENODEV;
	}
	...
	if (fw->size < sizeof(struct audio_config_database)) {
		printk(KERN_ERR "acdb: undersized database\n");
		goto fail;
	}
	if (strcmp(db->magic, "ACDB1.0")) {
		printk(KERN_ERR "acdb: invalid magic\n");
		goto fail;
	}
	if (db->entry_count > 1024) {
		printk(KERN_ERR "acdb: too many entries\n");
		goto fail;
	}
	if (fw->size < (sizeof(struct audio_config_database) +
			db->entry_count * sizeof(struct audio_config_data))) {
		printk(KERN_ERR "acdb: undersized TOC\n");
		goto fail;
	}
	for (n = 0; n < db->entry_count; n++) {
		if (db->entry[n].length > 4096) {
			printk(KERN_ERR "acdb: entry %d too large (%d)\n",
			       n, db->entry[n].length);
			goto fail;
		}
		if ((db->entry[n].offset + db->entry[n].length) > fw->size) {
			printk(KERN_ERR "acdb: entry %d outside of data\n", n);
			goto fail;
		}
	}
	if (htc_acdb_data)
...
static int acdb_get_config_table(uint32_t device_id, uint32_t sample_rate)
{
	...
	for (n = 0; n < db->entry_count; n++) {
		...
		if (n == db->entry_count) {
		printk(KERN_INFO "acdb: no entry for device %d, rate %d.\n",
		       device_id, sample_rate);
		return 0;
	}

	printk(KERN_INFO "acdb: %d bytes for device %d, rate %d.\n",
...
static int __init acdb_init(void)
{
	...
	if (IS_ERR(acdb_data.cb_thread_task)) {
		printk(KERN_ERR "ACDB=> Could not register cb thread\n");
		result = -ENODEV;
		goto err;
	}
...
s32 acdb_calibrate_device(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (result < 0) {
			mutex_unlock(&acdb_data.acdb_mutex);
			printk(KERN_ERR "Not able to get calibration data, continue\n");
			continue;
		}
...
err4:
	...
	if (result)
		printk(KERN_ERR "ACDB=> Could not unregister audpreproc callback\n");
err3:
	result = audpp_unregister_event_callback(&acdb_data.audpp_cb);
	if (result)
		printk(KERN_ERR "ACDB=> Could not unregister audpp callback\n");
err2:
	result = auddev_unregister_evt_listner(AUDDEV_CLNT_AUDIOCAL, 0);
	if (result)
		printk(KERN_ERR "ACDB=> Could not unregister device callback\n");
err1:
...
s32 initialize_rpc(void)
{
	...
	if (result) {
		printk(KERN_ERR "ACDB=> Device Attach failed\n");
		result = -ENODEV;
		goto done;
	}
done:
...
s32 initialize_memory(void)
{
	...
	if (acdb_data.virt_addr == NULL) {
		printk(KERN_ERR "ACDB=> Could not allocate acdb buffer\n");
		result = -ENOMEM;
		goto done;
	}
	...
	if (acdb_data.device_info == NULL) {
		printk(KERN_ERR "ACDB=> Could not allocate device controller memory\n");
		result = -ENOMEM;
		goto done;
	}
	...
	if (acdb_data.pp_iir == NULL) {
		printk(KERN_ERR "ACDB=> Could not allocate postproc iir memory\n");
		result = -ENOMEM;
		goto done;
	}
	...
	if (acdb_data.pp_mbadrc == NULL) {
		printk(KERN_ERR "ACDB=> Could not allocate postproc mbadrc memory\n");
		result = -ENOMEM;
		goto done;
	}
	...
	if (acdb_data.preproc_agc == NULL) {
		printk(KERN_ERR "ACDB=> Could not allocate preproc agc memory\n");
		result = -ENOMEM;
		goto done;
	}
	...
	if (acdb_data.preproc_iir == NULL) {
		printk(KERN_ERR "ACDB=> Could not allocate preproc iir memory\n");
		result = -ENOMEM;
		goto done;
	}
done:
...
s32 register_device_cb(void)
{
	...
	if (result) {
		printk(KERN_ERR "ACDB=> Could not register device callback\n");
		result = -ENODEV;
		goto done;
	}
done:
...
s32 register_audpp_cb(void)
{
	...
	if (result) {
		printk(KERN_ERR "ACDB=> Could not register audpp callback\n");
		result = -ENODEV;
		goto done;
	}
done:
...
s32 register_audpreproc_cb(void)
{
	...
	if (result) {
		printk(KERN_ERR "ACDB=> Could not register audpreproc callback\n");
		result = -ENODEV;
		goto done;
	}
...
s32 acdb_get_calibration(void)
{
	...
	if (the_ops->support_skip_get_acdb_table) {
		...
		printk(KERN_INFO "ACDB: skip_get_acdb_table = %d\n",
	...
	if (sz > 0) {
		...
		do {
			...
			if (result < 0) {
				printk(KERN_ERR "ACDB=> Device table RPC failure"
					" result = %d\n", result);
				result = -EINVAL;
				goto done;
			}
			...
			if (acdb_data.acdb_result.result == ACDB_RES_BADSTATE) {
				...
				acdb_data.acdb_state |= CAL_DATA_READY;
				acdb_data.enable = 1;
				printk(KERN_INFO "%d: change acdb_State to %d\n", __LINE__,
					acdb_data.acdb_state);
				goto done;
			} else {
				printk(KERN_ERR "ACDB=> modem failed to fill acdb values,"
		...
		printk(KERN_ERR "ACDB=> AUDCAL SW on modem is not"
...
s32 acdb_calibrate_audpp(void)
{
	...
	ret = get_lpa_session();
	if (ret && !apply_copp4) {
		printk(KERN_INFO "lpa is enabled sessions = %x\n", acdb_data.device_info->sessions);
		apply_copp4 = 1;
		copp4_applied = 0;
	...
	if (result) {
		printk(KERN_ERR "ACDB=> Failed to send IIR data to postproc\n");
		result = -EINVAL;
		goto done;
	} else
		printk(KERN_INFO "AUDPP is calibrated with IIR parameters"
					" for COPP ID %d\n",
	...
	if (result) {
		printk(KERN_ERR "ACDB=> Failed to send MBADRC data to"
					" postproc\n");
		result = -EINVAL;
		goto done;
	} else
		printk(KERN_INFO "AUDPP is calibrated with MBADRC parameters"
					" for COPP ID %d\n",
					acdb_data.device_info->dev_id);
	}
...
s32 acdb_fill_audpp_iir(void)
{
	...
	if (acdb_iir == NULL) {
		printk(KERN_INFO "unable to find  audpp iir block returning\n");
		return -1;
	}
					...
					if (prs_hdr->iid ==
						...
						*phy_addr = acdb_data.phys_addr\
						...
						if (acdb_data. \
							...
							printk(KERN_ERR "mbadrc bands number\
								too much.");
							return;
						}
						memcpy(acdb_data. \
...
s32 acdb_fill_audpp_mbadrc(void)
{
	...
	if (IS_ERR_VALUE(mbadrc_phys_addr)) {
		printk(KERN_ERR "failed to get mbadrc block\n");
		return -1;
	}
	...
	if (acdb_data.mbadrc_block.parameters.mbadrc_num_bands > mbadrc_num_bands) {
		printk(KERN_ERR "mbadrc bands number too much.");
		return -1;
	}
...
s32 acdb_calibrate_audpreproc(void)
{
	...
	if (result) {
		printk(KERN_INFO "ACDB=> Failed to send AGC data to preproc)\n");
		result = -EINVAL;
		goto done;
		} else
	...
	if (result) {
		printk(KERN_ERR "ACDB=> Failed to send IIR data to preproc\n");
		result = -EINVAL;
		goto done;
		} else
...
static void __exit acdb_exit(void)
{
	...
	if (result)
		printk(KERN_ERR "ACDB=> Could not stop kthread\n");
