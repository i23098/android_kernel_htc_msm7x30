		...
		if (mod_id == sptr->mod_to_q_tbl[i].module)
			if (q_idx == sptr->mod_to_q_tbl[i].q_type) {
				if (size <= sptr->mod_to_q_tbl[i].q_max_len)
					return 0;
				printk(KERN_ERR "q_idx: %d is not a valid queue \
					for module %x\n", q_idx, mod_id);
				return -EINVAL;
			}
	printk(KERN_ERR "cmd_buf size is more than allowed size\n");
	return -EINVAL;
}
				      ...
				      struct msm_adsp_module *adsp_module)
{
	...
	if (err < 0)
		printk(KERN_ERR "ADSP command send Failed\n");
...
static void  msm_get_init_info(void)
{
	...
	err = dalrpc_fcn_5(DALDEVICE_ADSP_CMD_IDX | 0x80000000,
							...
							&cmd, sizeof(cmd));
	if (err < 0)
		printk(KERN_ERR "INIT_INFO command send Failed\n");
}
		 ...
		 struct msm_adsp_ops *ops, void *driver_data)
{
	...
	*out = module;
	...
	if (rc) {
		...
		*out = NULL;
		printk(KERN_ERR "REGISTER_APP failed\n");
		mutex_unlock(&module->lock);
		goto done;
	}

	printk(KERN_INFO "module %s has been registered\n", module->name);
...
void msm_adsp_put(struct msm_adsp_module *module)
{
	...
	mutex_lock(&module->lock);
	if (module->ops) {
		printk(KERN_INFO "closing module %s\n", module->name);
		...
		if (module->state != ADSP_STATE_DISABLED) {
			printk(KERN_INFO "disabling module %s\n", module->name);
			mutex_unlock(&module->lock);
			msm_adsp_disable(module);
			return;
		}
	} else {
		printk(KERN_INFO "module %s is already closed\n", module->name);
	}
	mutex_unlock(&module->lock);
}
		   ...
		   void *cmd_buf, size_t cmd_size)
{
	...
	if (!module || !cmd_buf) {
		printk(KERN_ERR "Called with NULL parameters\n");
		return -EINVAL;
	}
	info = module->info;
	...
	if (module->state != ADSP_STATE_ENABLED) {
		...
		printk(KERN_INFO "module %s not enabled before write\n", module->name);
		return -ENODEV;
	}
	if (adsp_validate_module(module->id)) {
		spin_unlock_irqrestore(&adsp_write_lock, flags);
		printk(KERN_ERR "module id validation failed %s  %d\n",
				module->name, module->id);
		return -ENXIO;
	}
	if (dsp_queue_addr >= QDSP_MAX_NUM_QUEUES) {
		spin_unlock_irqrestore(&adsp_write_lock, flags);
		printk(KERN_ERR "Invalid Queue Index: %d\n", dsp_queue_addr);
		return -ENXIO;
	}
			...
			void *cmd_buf, size_t cmd_size)
{
	...
	if (retries > 20)
		printk(KERN_INFO "%s command took %d attempts: rc %d\n",
					...
					void *evt_buf, uint32_t len)
{
	...
	if (!init_info_completed && event == RPC_ADSP_RTOS_INIT_INFO) {
		...
		if (next_init_info_byte &&
			...
			printk(KERN_ERR "INIT_INFO - expecting next byte to be %d\n"
	...
	if (event == RPC_ADSP_RTOS_INIT_INFO) {
		printk(KERN_INFO "INIT_INFO Event\n");
	...
	if (args != NULL) {
		...
		printk(KERN_INFO "rpc event=%d, proc_id=%d, module=%d, image=%d\n",
			event, proc_id, module_id, image);

		module = find_adsp_module_by_id(&adsp_info, module_id);
		if (!module) {
			printk(KERN_ERR "module %d is not supported!\n", module_id);
			return;
		}
	} else {
		printk(KERN_ERR "NULL args\n");
		return;
	}
	...
	case RPC_ADSP_RTOS_MOD_READY:
		printk(KERN_INFO "module %s: READY\n", module->name);
		module->state = ADSP_STATE_ENABLED;
		wake_up(&module->state_wait);
		adsp_set_image(module->info, image);
		break;
	case RPC_ADSP_RTOS_MOD_DISABLE:
		printk(KERN_INFO "module %s: DISABLED\n", module->name);
		module->state = ADSP_STATE_DISABLED;
		wake_up(&module->state_wait);
		break;
	case RPC_ADSP_RTOS_SERVICE_RESET:
		printk(KERN_INFO "module %s: SERVICE_RESET\n", module->name);
		module->state = ADSP_STATE_DISABLED;
		wake_up(&module->state_wait);
		break;
	case RPC_ADSP_RTOS_CMD_SUCCESS:
		printk(KERN_INFO "module %s: CMD_SUCCESS\n", module->name);
		break;
	case RPC_ADSP_RTOS_CMD_FAIL:
		printk(KERN_INFO "module %s: CMD_FAIL\n", module->name);
		break;
	case RPC_ADSP_RTOS_DISABLE_FAIL:
		printk(KERN_INFO "module %s: DISABLE_FAIL\n", module->name);
		break;
	default:
		printk(KERN_ERR "unknown event %d\n", event);
		mutex_unlock(&module->lock);
		return;
	}
#ifdef CONFIG_MSM_ADSP_REPORT_EVENTS
	...
	struct adsp_info *info, void *dsp_addr)
{
	...
	if (rtos_task_id > info->max_task_id) {
		printk(KERN_ERR "bogus task id %d\n", rtos_task_id);
		return 0;
	}
	...
	if (!module) {
		printk(KERN_ERR "no module for task id %d\n", rtos_task_id);
		return 0;
	}
	...
	if (!module->ops) {
		printk(KERN_ERR "module %s is not open\n", module->name);
		return 0;
	}
...
static int adsp_get_event(struct adsp_info *info)
{
	...
	printk(KERN_ERR "not ready after 100uS\n");
	rc = -EBUSY;
	goto done;
...
ready:
	...
	if (cmd_type != ADSP_RTOS_READ_CTRL_WORD_CMD_TASK_TO_H_V) {
		printk(KERN_ERR "unknown dsp cmd_type %d\n", cmd_type);
		rc = -EIO;
		goto done;
	}
...
static irqreturn_t adsp_irq_handler(int irq, void *data)
{
	...
	if (cnt == 15)
		printk(KERN_ERR "too many (%d) events for single irq!\n", cnt);
	return IRQ_HANDLED;
}
...
int msm_adsp_enable(struct msm_adsp_module *module)
{
	...
	printk(KERN_INFO "enable '%s'state[%d] id[%d]\n",
	...
	case ADSP_STATE_DISABLED:
		...
		if (module->state == ADSP_STATE_ENABLED) {
			...
			printk(KERN_ERR "module '%s' enable timed out\n", module->name);
			rc = -ETIMEDOUT;
		}
	...
	case ADSP_STATE_DISABLING:
		printk(KERN_ERR "module '%s' disable in progress\n", module->name);
		rc = -EBUSY;
		break;
	}
	mutex_unlock(&module->lock);
...
static int msm_adsp_probe(struct platform_device *pdev)
{
	...
	adsp_info.send_irq = MSM_AD5_BASE + adsp_info.send_irq_shift;
	adsp_info.read_ctrl = MSM_AD5_BASE + adsp_info.read_ctrl_shift;
	adsp_info.write_ctrl = MSM_AD5_BASE + adsp_info.write_ctrl_shift;
	...
	if (rc) {
		printk(KERN_ERR "adsp attach failed : %d\n", rc);
	...
	if (adsp_info.cb_handle == NULL) {
		printk(KERN_ERR "Callback registration failed\n");
	...
	rc = wait_event_timeout(adsp_info.init_info_wait,
		...
		10 * HZ);
	if (!rc) {
		printk(KERN_ERR "INIT_INFO failed\n");
		rc = -ETIMEDOUT;
	} else
		return 0;
